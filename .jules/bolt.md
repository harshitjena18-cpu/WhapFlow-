...
## 2024-05-22 - [Environment Access Optimization]
**Learning:** Evaluated branching for environment variable access (Deno vs Node.js) on every call in high-traffic utilities like `getEnv` adds measurable overhead (~0.05µs per call). Short-circuiting lookups with optional chaining (`g.Deno?.env?.get?.(key) ?? g.process?.env?.[key]`) provides a ~10% performance boost while maintaining environment flexibility.
**Action:** Use short-circuiting nullish coalescing for environment lookups. Consolidate duplicate function exports to prevent bundler bloat and satisfy strict Deno linting rules.
