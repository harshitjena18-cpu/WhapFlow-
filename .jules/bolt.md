...
## 2026-06-02 - [CORS Middleware Optimization]
**Learning:** Reordering conditional checks in hot-path middlewares like CORS to prioritize static string comparisons (`===`) over regular expressions (`RegExp.test()`) can reduce the latency of that specific block by ~75%. This is especially impactful for Edge Functions where every millisecond of CPU time counts towards execution limits and response latency.
**Action:** Always audit request-level middlewares (CORS, Auth, Routing) and move simple string-based validation to the top, deferring regex or I/O-bound checks.
