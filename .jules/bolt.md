...
## 2026-04-10 - [Validation Sync & CORS Hot-path Optimization]
**Learning:** Using `useEffect` + `useState` to sync validation state based on input triggers two render cycles per keystroke (Change -> Render -> Effect -> State Update -> Re-render). Reordering CORS checks to put string equality before regex avoids expensive regex execution for the majority of production traffic.
**Action:** Prefer `useMemo` for derived state (like validation) to ensure it's computed during the initial render. Always prioritize fast string comparisons over regex in middleware hot-paths.
