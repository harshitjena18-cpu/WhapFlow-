...
## 2026-04-03 - [Optimized Hex Decoding for HMAC]
**Learning:** Manual hex-to-bytes conversion loops in JavaScript (using substring/parseInt) are significantly slower (~11x) than native decoding provided by `Buffer.from(str, 'hex')`. In high-traffic paths like OAuth callbacks and webhooks, this overhead adds up across concurrent requests.
**Action:** Always prefer `Buffer.from(str, 'hex')` or `Buffer.from(str, 'base64')` for decoding cryptographic signatures and headers instead of manual loops.
