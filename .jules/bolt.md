...
## 2024-05-21 - [CORS Origin Optimization]
**Learning:** Strict string equality checks ('===') are ~4.5x faster than regex testing ('.test()') for origin validation in the Deno runtime. Prioritizing production origins in middleware reduces the latency floor for the vast majority of requests.
**Action:** Always prioritize high-frequency static string matches over regular expressions in request hot-paths (middleware, routing).
