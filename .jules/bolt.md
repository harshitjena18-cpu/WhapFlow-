...
## 2026-03-27 - [Derived Validation State to Avoid Double Renders]
**Learning:** Performing input validation on every keystroke using a `useEffect` + `useState` pattern in React components causes an immediate second render cycle for each update, introducing noticeable lag on high-interaction forms.
**Action:** Refactor input validation to use the synchronous `useMemo` pattern. This derives the validation state synchronously during the initial render phase, reducing the render count from 2 per keystroke to 1 and vastly improving responsiveness.
