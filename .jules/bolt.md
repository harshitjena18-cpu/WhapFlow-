...
## 2024-05-21 - [CORS & Env Hot-path Optimizations]
**Learning:** Middleware and utility functions called on every request (like CORS validation and `getEnv`) are highly sensitive to micro-latencies. Reordering checks to prioritize static string comparisons over regex in CORS, and using module-level constants for environment detection in `getEnv`, reduces the per-request latency floor.
**Action:** Always prioritize static string checks over regex in hot-paths. Cache environment detection results at the module level to avoid redundant property lookups on `globalThis`.
