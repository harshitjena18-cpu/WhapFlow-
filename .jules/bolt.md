...
## 2026-03-25 - [Derived State for Reactive Validation]
**Learning:** In highly interactive React views with rich text inputs (like `TemplatesView.tsx`), using `useEffect` with multiple state setters to synchronize validation states triggers a redundant and expensive full re-render cycle per keystroke. Converting this pattern to synchronous `useMemo`-based derived state completely eliminates the secondary re-render path, improving key-input response times.
**Action:** When performing local/on-the-fly validation or state derivations, prefer `useMemo` or raw derived declarations over `useState` + `useEffect` synchronizations.
