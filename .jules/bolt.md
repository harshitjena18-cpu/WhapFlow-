...
## 2026-04-05 - [Atomic Queue Batch Claiming]
**Learning:** In a high-concurrency job queue, individual "lock-and-claim" operations via `kv.del` or `kv.update` create a massive database round-trip bottleneck. By using PostgreSQL's atomic `DELETE ... RETURNING` (via Supabase's `.delete().in(keys).select('key')`), we can claim an entire batch of $N$ jobs in exactly ONE round-trip while maintaining strict atomicity. This reduces the claiming wall-clock time by ~95% for full batches.
**Action:** When processing batches of work that require atomic ownership, use `claimBatch` patterns to minimize I/O overhead.
