...
## 2024-05-21 - [Middleware Hot-path String Optimization]
**Learning:** In request-level middleware like CORS, regex testing (`RegExp.test`) is significantly slower (~15x) than direct string equality (`===`). Since production traffic primarily originates from a known `APP_DOMAIN`, prioritizing string equality before falling back to regex for localhost reduces the latency floor for all production requests.
**Action:** Always prioritize string equality checks for known production domains in hot-path middlewares before executing more complex validation logic like regex.
