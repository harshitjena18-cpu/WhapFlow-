...
## 2026-03-21 - [High-Contrast Focus Indicators, Dynamic Toggle ARIA, and Icon Tooltips]
**Learning:** Using low-contrast focus ring opacities (such as `/20`) on light/white backgrounds fails to satisfy WCAG accessibility contrast requirements, hindering keyboard-navigating users. Elevating these rings to `/50` opacity vastly improves focus visibility. Additionally, icon-only buttons like the global notification bell need both an `aria-label` and a visual `Tooltip` for sighted keyboard/hover users. Lastly, mobile-menu toggle buttons must use state-driven dynamic `aria-label`s (transitioning between "Open menu" and "Close menu"), `aria-expanded`, and `aria-controls` for standard-compliant screen-reader behavior.
**Action:** Always enforce at least `/50` opacity (or solid color) for focus-visible rings on light/white backgrounds. Always pair icon-only triggers with descriptive Radix-based `Tooltip`s. Ensure all toggle states (like responsive menus) are explicitly communicated with dynamic ARIA attributes.
