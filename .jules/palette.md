...
## 2026-06-16 - [Dynamic Accessibility State and Visible Focus for Icon-Only Buttons]
**Learning:** Icon-only layout control buttons (like mobile menu toggles and notification bell indicators) require both standard Tooltips for sighted users and robust, state-driven ARIA labels (e.g., transitioning between "Open menu" and "Close menu") for screen readers. Furthermore, adding micro-animations like `active:scale-95` and high-contrast, theme-matching keyboard focus rings (using at least `/50` opacity or solid values) significantly improves both accessibility compliance and tactile feedback.
**Action:** When creating or modifying header or sidebar layout buttons, always ensure they are wrapped in a `Tooltip` with proper focus tracking, have a dynamic `aria-label` mapped to state, and feature an explicit `focus-visible:ring-2` indicator with safe color contrast.
