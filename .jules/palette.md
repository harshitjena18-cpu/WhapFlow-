...
## 2026-03-21 - [Focus Contrast and Header Tooltips]
**Learning:** Icon-only utility buttons in application headers are frequently missed by keyboard accessibility audits, leaving them without visible focus outlines or descriptive hover states. Additionally, when designing focus indicators for light or white background themes, low-contrast opacities (like `focus-visible:ring-purple-500/20`) fail WCAG accessibility standards. Boosting the focus ring opacity to at least `/50` ensures proper contrast and clear tactile awareness during keyboard navigation.
**Action:** Wrap all header icon-only buttons in accessible tooltips (`Tooltip`) to provide visual explanations on hover/focus. When applying focus indicators on light background containers, always use at least a `/50` opacity (or a solid primary color) to ensure keyboard path visibility.
