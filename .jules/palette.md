...
## 2026-06-20 - [Shortcut Discoverability and Form Accessibility in Dialogs]
**Learning:** Keyboard shortcuts (like `Cmd/Ctrl + Enter`) are powerful for efficiency but often invisible to the average user. Surfacing these via visual `<kbd>` hints on the primary action button, combined with `aria-keyshortcuts`, makes them discoverable and accessible. Furthermore, ensuring that form inputs are programmatically linked to their help and validation text via `aria-describedby` is essential for screen reader users to understand the context and errors correctly.
**Action:** Always surface primary modal shortcuts using a visual `<kbd>` hint. Use `aria-describedby` to link all form inputs with their associated help text, character counters, and validation messages, ensuring these containers remain in the DOM even when empty to maintain link stability.
