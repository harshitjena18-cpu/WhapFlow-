...
## 2026-06-12 - [Interactive Creation Dialog Accessibility]
**Learning:** In complex creation dialogs (like AI Template Generators), purely visual cues for character limits and validation errors are insufficient for screen reader users. Explicitly linking labels to inputs using `htmlFor` and `id`, and associating text areas with validation message containers via `aria-describedby` (ensuring the container remains in the DOM), provides a robust semantic connection. Additionally, making character counters "live" using `aria-live="polite"` ensures users receive immediate feedback on their input length.
**Action:** Always verify that every input has a programmatically associated label. Use `aria-describedby` to link inputs to helper text or validation messages. Ensure character counters use `aria-live` to provide real-time status updates without interrupting the user's flow.
