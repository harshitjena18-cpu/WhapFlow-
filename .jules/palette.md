...
## 2026-07-10 - [Context-Aware Toggle Labels and Utility Discoverability]
**Learning:** Static ARIA labels on toggle buttons (e.g., "Toggle menu") are less descriptive than context-aware labels that reflect the current UI state (e.g., "Open menu" vs "Close menu"). Providing this dynamic feedback improves clarity for screen reader users. Additionally, utility icons in headers (like notifications) often lack immediate context for sighted users; wrapping them in `Tooltip` components ensures they are discoverable without adding visual clutter.
**Action:** Use dynamic `aria-label` attributes for all stateful toggle components. Ensure all icon-only utility buttons in global navigation are wrapped in `Tooltip` components to maintain functional parity for all users.
