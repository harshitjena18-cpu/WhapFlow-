...
## 2026-03-21 - [High-Contrast Focus Indicators and State-Aware Mobile Toggles]
**Learning:** When using focus rings on light/white interfaces (such as sidebars or headers), low-contrast opacities like `/20` are virtually invisible to users with visual impairments, failing WCAG standards. Boosting opacity to `/50` or using solid theme colors dramatically enhances key navigation clarity. Furthermore, a mobile sidebar toggle must maintain dynamic semantic state (such as `aria-expanded`, dynamic `aria-label`, and `aria-controls` linked to a stable ID) to ensure screen readers can announce dynamic navigation transitions.
**Action:** Always use at least `/50` opacity (or solid borders) for focus rings on high-brightness backgrounds. Always equip mobile toggle controls with dynamic state attributes synchronized with the toggle's open/closed state.
