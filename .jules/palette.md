...
## 2026-03-24 - [Accessible Header and Mobile Navigation Toggles]
**Learning:** Icon-only buttons (like notification bells or mobile menu toggles) without explicit Tooltips and keyboard focus indicators present a major accessibility barrier for sighted keyboard users. Standardizing on high-contrast focus rings (`focus-visible:ring-purple-500/50`) on light/white backgrounds and adding dynamic state-driven Tooltips & ARIA attributes (`aria-expanded`, `aria-controls`, and dynamic `aria-label`) ensures a seamless, modern, and highly-discoverable user experience for all interaction modes.
**Action:** Always wrap icon-only header/sidebar actions in standard `Tooltip` components. Ensure focus indicators are high-contrast (avoiding low-contrast opacities like `/20` on light backdrops). For dynamic toggles, always implement state-driven ARIA labels and properties matching their associated containers.
