...
**Learning:** Performance-driven caching in cryptographic modules is extremely dangerous if the cache key (the secret) does not represent all entropy used in derivation (the salt). Reusing a key with the wrong salt leads to decryption failures and potential security undefined behavior.
