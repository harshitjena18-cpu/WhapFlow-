...
## 2025-05-25 - [Timing Attack in WhatsApp Webhook and CI-Blocking Duplications]
**Vulnerability:** WhatsApp webhook verification used insecure equality operators for the verify token. Additionally, duplicate utility declarations in backend files were blocking security verification tools and CI.
**Learning:** Standard equality comparisons for secrets are vulnerable to timing attacks. Furthermore, redundant code in autogenerated or core files (like `kv_store.tsx`) can break toolchains (like `tsx`) that enforce strict declaration rules.
**Prevention:** Always use `secureCompare` for secret verification. Regularly audit core utilities for duplicate declarations to maintain toolchain compatibility and security posture.
