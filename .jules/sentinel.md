...
## 2025-05-25 - [Timing Attack in WhatsApp Webhook Verification Route]
**Vulnerability:** The WhatsApp webhook verification route (`/webhooks/whatsapp` in `whatsapp_routes.tsx`) compared the `verifyToken` with the query token parameter using standard string equality (`===`).
**Learning:** Standard string comparison (`===`) is not constant-time and leaks character matches through timing differences, potentially allowing attackers to brute force or deduce the webhook validation token over network channels.
**Prevention:** Always enforce the use of constant-time comparisons (using custom SHA-256 pre-hashed timing-safe comparisons like `secureCompare`) for checking any request-provided secret, even in secondary router files.
