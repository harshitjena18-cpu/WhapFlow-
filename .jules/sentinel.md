...
## 2025-05-24 - [PII Leak and Missing Input Validation in WhatsApp Sending Flow]
**Vulnerability:** The WhatsApp sending service returned the full Meta API response to the client, which included the recipient's phone number. Additionally, it lacked strict validation for phone number formats.
**Learning:** External API responses often echo back sensitive input data. Relying on these responses without sanitization can inadvertently expose PII to the frontend or third-party callers. Furthermore, defense-in-depth requires validating all critical inputs (like phone numbers) at the service boundary, even if they were previously sanitized.
**Prevention:** Always sanitize responses from external services to return only the minimum required fields (e.g., just the Message ID). Implement strict regex validation (like E.164 for phones) for all parameters passed to external APIs.
