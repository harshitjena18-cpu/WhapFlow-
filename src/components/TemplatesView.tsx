            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2 h-full flex flex-col">
                ...
                <Textarea 
                  id="content"
                  ref={contentRef}
                  aria-describedby="content-counter validation-messages"
                  ...
                  className={cn(
                    "flex-1 min-h-[150px] font-mono text-sm resize-none bg-gray-50",
                    validationErrors.length > 0 ? 'border-red-300 focus:ring-red-200' : ''
                  )}
                ...
                {/* Validation Messages */}
                <div id="validation-messages" className="space-y-2 mt-1 min-h-[20px]">
                  {validationErrors.map((err, idx) => (
                    <div key={`err-${idx}`} className="text-[10px] text-red-600 bg-red-50 p-2 rounded flex items-start gap-1.5 border border-red-100 animate-in fade-in slide-in-from-top-1">
                      <AlertCircle className="w-3 h-3 flex-shrink-0 mt-0.5" />
                      <span>{err}</span>
                    </div>
                  ))}
                  {validationWarnings.map((warn, idx) => (
                    <div key={`warn-${idx}`} className="text-[10px] text-orange-600 bg-orange-50 p-2 rounded flex items-start gap-1.5 border border-orange-100 animate-in fade-in slide-in-from-top-1">
