...
// PERFORMANCE: Hoist encoders and static values to module level to reduce redundant allocations
 ...
 * PERFORMANCE: Implements thundering herd protection using promise-based caching.
 */
function getV3Key(): Promise<CryptoKey> {
  ...
  _v3KeyPromise = (async () => {
    try {
      ...
      // Keep _cachedV3Key, but clear the promise once it's resolved/rejected
      _v3KeyPromise = null;
    }
  })();
...
export async function encrypt(text: string | null | undefined): Promise<string | null | undefined> {
  ...
  try {
    ...
    // PERFORMANCE: Avoid redundant Uint8Array wrapping by passing ArrayBuffer directly to b64
...
// PERFORMANCE: Updated to accept ArrayBuffer | Uint8Array to minimize redundant allocations
