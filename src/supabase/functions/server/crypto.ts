...
// PERFORMANCE: Hoist encoder and pre-encode static HKDF parameters to eliminate redundant overhead per call
 ...
 * during concurrent cold-starts, ensuring the expensive HKDF derivation only runs once.
 */
...
export async function encrypt(text: string | null | undefined): Promise<string | null | undefined> {
  ...
  try {
    ...
    // PERFORMANCE: b64 helper now accepts ArrayBuffer directly, avoiding redundant Uint8Array wrapping of ciphertext
...
// PERFORMANCE: Optimized b64 helper to accept ArrayBuffer | Uint8Array directly
