 ...
 *
 * PERFORMANCE: By hashing inputs first, we can use timingSafeEqual on fixed-length digests,
 * which is more efficient than character-by-character comparison of variable-length strings.
 */
export function secureCompare(a: string | null | undefined, b: string | null | undefined): boolean {
  ...
  // We intentionally omit a direct length check (a.length === b.length) to prevent length leakage.
