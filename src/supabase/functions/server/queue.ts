...
export async function processPendingJobs<T = unknown>(handler: (payload: T) => Promise<void>) {
  ...
  // PERFORMANCE: Atomic batch-claiming of all due jobs in a single database round-trip.
  // This reduces N database requests to 1, drastically improving efficiency under load.
  const jobKeys = (dueJobs as Job<T>[]).map(j => j.key);
  ...
  console.log(`[Queue] Successfully claimed ${claimedJobs.length} jobs.`);
