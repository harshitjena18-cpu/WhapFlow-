...
export async function processPendingJobs<T = unknown>(handler: (payload: T) => Promise<void>) {
  ...
  // Instead of claiming one-by-one, we claim the entire batch in 1 database round-trip.
  // This reduces latency by O(N) where N is the batch size.
  const dueKeys = dueJobs.map(j => (j as Job<T>).key);
  const claimedKeys = await kv.claimBatch(dueKeys);
  const claimedKeysSet = new Set(claimedKeys);

  // Filter jobs to only those we successfully claimed
  const jobsToProcess = (dueJobs as Job<T>[]).filter(j => claimedKeysSet.has(j.key));

  console.log(`[Queue] Successfully claimed ${jobsToProcess.length} jobs.`);

  if (jobsToProcess.length === 0) return;

  // Process claimed jobs concurrently with a limit to avoid overwhelming resources
