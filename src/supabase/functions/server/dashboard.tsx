...
dashboardApp.get("/data", async (c) => {
  // SECURITY: Use verified shop domain from session token context (enforced by middleware).
  // Relying solely on 'verified_shop' prevents IDOR via untrusted query parameter fallbacks.
  const shop = c.get("verified_shop") as string;

  if (!shop) return c.json({ error: "Unauthorized: Verified shop context missing" }, 401);
...
dashboardApp.get("/automations", async (c) => {
  // SECURITY: Use verified shop domain exclusively from session context
  const shop = c.get("verified_shop") as string;
