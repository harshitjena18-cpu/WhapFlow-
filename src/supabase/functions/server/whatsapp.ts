...
export const sendWhatsAppTemplate = async ({
  ...
  try {
    ...
    // SECURITY: Return only necessary data to avoid leaking PII (like wa_id)
    return { success: true, data: { wamid }, wamid };
  } catch (error) {
    const message = getErrorMessage(error);
    console.error("❌ Network/Server Error sending WhatsApp:", message);
