...
authApp.post("/signup", async (c) => {
  ...
  // SECURITY: Use constant-time comparison to prevent timing attacks on the service role key
  const isAuthorized = authHeader && serviceKey && secureCompare(authHeader, `Bearer ${serviceKey}`);
