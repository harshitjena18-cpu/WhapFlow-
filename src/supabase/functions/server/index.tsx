...
app.post(`${SERVER_BASE_PATH}/api/whatsapp/send`, async (c) => {
  try {
    ...
    const [isWhatsappAuth, isServiceAuth] = await Promise.all([
      whatsappApiKey && authHeader ? secureCompare(authHeader, `Bearer ${whatsappApiKey}`) : Promise.resolve(false),
      serviceRoleKey && authHeader ? secureCompare(authHeader, `Bearer ${serviceRoleKey}`) : Promise.resolve(false)
    ]);
...
app.get(`${SERVER_BASE_PATH}/api/webhooks/whatsapp`, async (c) => {
  ...
  const isVerified = mode === "subscribe" && verifyToken && token && await secureCompare(token, verifyToken);
